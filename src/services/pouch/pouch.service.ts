...
export class PouchService {
  ...
  private tvShowsLiveReplication: any;
  private bufferedTvShowsLiveReplicationTo: any;
  private bufferedTvShowsLiveReplicationFrom: any;
  

  public initialisation$: BehaviorSubject<boolean>;

  public moviesInView: Array<PouchMovie | BufferedPouchMovie> = [];
  ...
  public async initialize(): Promise<void> {
    try { 
      //todo: refactor
      const pouchMovies = await this.setupMovieCollection();
      const bufferedMovies = await this.setupBufferedMovieCollection();
      const pouchTvShows = await this.setupTvShowCollection();
      const bufferedTvShows =  await this.setupBufferedTvShowsCollection();


      this.moviesInView = this.setupMoviesInView(bufferedMovies, pouchMovies);
  ...
  getMovie(tmdbId: number): PouchMovie | BufferedPouchMovie
  {
    
    const i = this.moviesInView.findIndex(m => m.tmdbId === tmdbId);

    if(i !== -1)
    {
      return this.moviesInView[i];
    }
    
  }


  getMoviesInView(): Array<BufferedPouchMovie | PouchMovie>
  {
    return this.moviesInView;
  }
  ...
  private setupMoviesInView(bufferedMovies: BufferedPouchMovie [], pouchMovies: PouchMovie[]): Array<PouchMovie | BufferedPouchMovie> {
  ...
  public async deleteMovieFromPouchAndLocal(movie: PouchMovie | BufferedPouchMovie)
  {
